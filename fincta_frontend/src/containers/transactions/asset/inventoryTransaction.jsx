...
const InventoryTransaction = (props) => {
  ...
  useEffect(() => {
    let inventoryIds = localStorage.getItem("inventoryIds");
    if (inventoryIds) {
      getInventoryTransactions(JSON.parse(inventoryIds));
    }
  }, []);
  ...
  const handleSeedInput = (e) => {
    let filteredSuggestions = inventoryTransactions.filter(
      (d) =>
        JSON.stringify(d)
          .replace(" ", "")
          .toLowerCase()
